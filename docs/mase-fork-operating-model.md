...
- Source fork: local code lives here and should be compared against the current
  upstream release branch, now `upstream/v7`, before claims about local
  changes.
...
- Keep upstream mirror branches such as `v7` clean.
...
git diff --stat upstream/v7..HEAD
git diff --name-status upstream/v7..HEAD
...
As last verified on 2026-05-13 against `upstream/v7`, the local fork carries
...
Refresh this section from `git diff upstream/v7..HEAD` before relying on it for
shipping decisions.
...
- `ARCHITECTURE.md`
- `git diff upstream/v7..HEAD` as of 2026-05-13
- `/Users/mase/.codex/docs/reference/graphify.md`
