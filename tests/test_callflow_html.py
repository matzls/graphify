...
from graphify.callflow_html import (
...
def test_relation_label_renders_unknown_free_form_relation():
    assert relation_label("writes", "en") == "writes"
    assert relation_label("records_to", "en") == "records to"


def test_write_callflow_html_renders_unknown_free_form_relation(tmp_path):
    out = _make_graphify_out(tmp_path)
    graph_path = out / "graph.json"
    try:
        graph = json.loads(graph_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AssertionError("callflow graph fixture should be valid JSON") from exc
    graph["links"] = [
        {
            "source": "run",
            "target": "api",
    ...
    html_path = write_callflow_html(
        tmp_path,
        output="graphify-out/free-form-relation.html",
        max_sections=4,
    )

    content = html_path.read_text(encoding="utf-8")
    assert "records to" in content
