...
def test_install_rejects_external_core_hooks_path(tmp_path):
    ...
    subprocess.run(
        ["git", "-C", str(repo), "config", "core.hooksPath", str(external_hooks)],
        check=True,
        capture_output=True,
    )

    with pytest.raises(
        RuntimeError, match="Refusing to install Graphify hooks into external core\\.hooksPath"
    ):
        install(repo)
...
def test_post_commit_hook_rebuilds_for_deleted_code_paths():
    assert "CODE_EXTENSIONS" in _HOOK_SCRIPT
    assert "if suffix in CODE_EXTENSIONS:" in _HOOK_SCRIPT
    assert "needs_code = True" in _HOOK_SCRIPT
                ...
                "hooks": {
                    "PreToolUse": [
                        ...
                        },
                        {
                            "matcher": "Bash",
                            "hooks": [
                                {"type": "command", "command": "cd /repos/graphify && ./lint.sh"}
                            ],
                        },
                    ]
                }
