...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("Commands:")
        print(
            "  install [--project] [--portable] [--platform P]  copy skill or project activation (claude|windows|codebuddy|codex|opencode|aider|amp|agents|claw|droid|trae|trae-cn|gemini|cursor|antigravity|hermes|kiro|pi|devin)"
        )
        ...
        print(
            "  codex-session-start [path]  emit Codex SessionStart JSON for pending semantic refresh"
        )
        ...
        print(
            "    --backend B             gemini|kimi|claude|openai|deepseek|ollama|pi (default: automatic)"
        )
        print(
        ...
        print("    --allow-image-upload    authorize fresh Pi raster pixels for this run")
        ...
        print(
            "    --portable             write exactly `graphify codex-session-start`; graphify must be on PATH"
        )
