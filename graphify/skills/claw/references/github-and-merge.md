...
```bash
graphify extract ./core/ --backend ollama      # → ./core/graphify-out/graph.json
graphify extract ./service/ --backend ollama   # → ./service/graphify-out/graph.json
graphify extract ./platform/ --backend ollama  # → ./platform/graphify-out/graph.json
# Include --model <value> only when the user supplied one; otherwise model
# resolution is explicit --model, then OLLAMA_MODEL, then Graphify's built-in
# default deepseek-v4-pro:cloud. Use another --backend only when the user
