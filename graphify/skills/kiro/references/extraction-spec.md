...
- AMBIGUOUS: uncertain but materially useful — flag it; otherwise omit.
...
- Prefer one canonical same-abstraction semantic concept while preserving useful code-symbol/document-concept separation. Omit document-root `references` edges that merely enumerate already-connected concepts, plus incidental nouns and low-value associations.
...
- Semantic similarity: if two concepts solve the same problem or represent the same idea without a structural link (no import, call, or citation), add a `semantically_similar_to` edge marked INFERRED with confidence_score 0.6-0.95. Materially useful, non-obvious cross-file links only.
