...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    for node, data in prefixed.nodes(data=True):
        existing_external = external_labels.get(data.get("label", ""))
        if not data.get("source_file") and existing_external:
            nodes_to_skip.add(node)
            endpoint_remap[node] = existing_external
    ...
    for u, v, data in prefixed.edges(data=True):
        u = endpoint_remap.get(u, u)
        v = endpoint_remap.get(v, v)
